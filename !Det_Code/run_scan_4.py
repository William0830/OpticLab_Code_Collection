...
def run_scan(xlim, ylim, z_focus, resolution, tacq, save_file, vmin, vmax, predelay=0, autozero=False, send_email=False):
    ...
    t_bigsteps = (len_x - 1) * (major_axis_wait_time) # time to move in major axis
    t_smallsteps = (len_x) * (len_y - 1) * (minor_axis_wait_time) # time to move in minor axis
    t_measure = (len_x) * (len_y) * (tacq/1000 + 0.045) # 45 ms is the time it takes to get the intensity from the HH400
    t_naught = 0.8 + 1.5 + 1 # piezo startup + HH400 startup + first move

    estimated_time = 1/60 * (t_bigsteps + t_smallsteps + t_measure + t_naught + predelay)

    print('Estimated Scan Time for {}: {} minutes'.format(scan_name, round(estimated_time, 4)))
    input('Press Enter to continue...')

    with GCSDevice(CONTROLLERNAME) as pidevice, HH400_Histo_Manager(send_error_email = send_email) as HH400, open(save_file, 'w') as scan_data_file:
        ...
        for i, x_pos in enumerate(xnodes):
            for j, y_pos in enumerate(ynodes):
                ...
                intensity_ij = HH400.integrate_intensity(tacq=tacq) # measure intensity for tacq ms
        ...
        print('Done')
        pidevice.MOV(OrderedDict([('1', 0), ('2', 0), ('3', z_focus)])) # move to min position
...
if __name__ == '__main__':
    ...
    rel_path = r'C:\Users\spmno\OneDrive\Documents\spm\test4\Scan Data\{}'.format(today)
    ...
    if show_plot:
        os.chdir(r"C:\Users\spmno\OneDrive\Documents\spm\test4\Scan Data\\" + datetime.today().strftime('%Y-%m-%d'))
